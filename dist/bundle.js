...
},{"accidental-value":1,"notecoord":5}],8:[function(require,module,exports){
...
},{"./interval":10,"./knowledge":11,"./note":12,"daccord":2}],10:[function(require,module,exports){
var knowledge = require('./knowledge');
...
global.app = function () {
  ...
  var playContent = function playContent(e) {
    new _player2.default('acoustic_grand_piano', input.value).start();
  };
  ...
  var dropBox = document.getElementById('content-input');
...
},{"./player":19}],17:[function(require,module,exports){
'use strict';
...
var BaseMapper = function () {
  function BaseMapper() {
    ...
    this.scales = ['major', 'minor', 'dorian', 'mixolydian', 'locrian', 'majorpentatonic', 'minorpentatonic', 'melodicminor'];
  }
...
var _baseMapper = require('./baseMapper');

var _baseMapper2 = _interopRequireDefault(_baseMapper);
...
var TextMapper = function (_BaseMapper) {
  _inherits(TextMapper, _BaseMapper);

  function TextMapper(text) {
    _classCallCheck(this, TextMapper);

    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(TextMapper).call(this));

    _this.text = text;
    return _this;
  }

  _createClass(TextMapper, [{
    key: 'createNotes',
    value: function createNotes(key, scale) {
      var basicScale = _teoria2.default.note(key).scale(scale).simple();
      var thirds = [basicScale[0], basicScale[2], basicScale[3], basicScale[4]];

      return thirds.concat(basicScale.map(function (note) {
        return note + '\'';
      })).concat(basicScale.map(function (note) {
        return note + '\'\'';
      })).concat(basicScale.map(function (note) {
        return note + '\'\'\'';
      })).concat(basicScale.map(function (note) {
    ...
    value: function map() {
      ...
      var sortedUniqueChars = this.sortedUniqueItems(chars, this.text.length);
      var mapping = this.mapTones(sortedUniqueChars);

      var tones = [];

      var modifiedChars = this.orderMostFrequentWords(chars);

      modifiedChars.forEach(function (char) {
        if (char.match(/[\.\?\!]+/)) {
          tones.push(-1);
        } else {
          tones.push(_teoria2.default.note(mapping[char]).midi());
        }
      });
    ...
    key: 'orderMostFrequentWords',
    value: function orderMostFrequentWords(text) {
      return text;
    }
  }, {
    key: 'mostFrequentWords',
    value: function mostFrequentWords() {
      var words = this.text.split(/\W/).filter(function (word) {
        return word != '';
      });
      return this.sortedUniqueItems(words, words.length);
    }
  }, {
    key: 'sortedUniqueItems',
    value: function sortedUniqueItems(items, totalCount) {
      var itemCounts = this.countAppearance(items);

      var normalize = function normalize(propagation, item) {
        propagation.push([item, itemCounts[item] / totalCount]);
        return propagation;
      };

      return Object.keys(itemCounts).reduce(normalize, []).sort(function (a, b) {
        return a[1] < b[1];
      }).map(function (a) {
        return a[0];
      });
    ...
    key: 'countAppearance',
    value: function countAppearance(items) {
      var count = function count(counts, item) {
    ...
    key: 'cleanText',
    value: function cleanText() {
      return this.text.replace(/\s/g, '');
    }
  }]);

  return TextMapper;
}(_baseMapper2.default);

exports.default = TextMapper;

},{"./baseMapper":17,"teoria":8}],19:[function(require,module,exports){
'use strict';
...
var _textMapper = require('./mappers/textMapper');
...
var Player = function () {
  ...
  _createClass(Player, [{
    ...
    value: function start() {
      ...
      MIDI.loadPlugin({
        ...
        onsuccess: function onsuccess() {
          ...
          var notes = $this.loadNotesFromContent();
          $this.playNotes(notes);
        }
      });
    }
  }, {
    key: 'loadNotesFromContent',
    value: function loadNotesFromContent() {
      return new _textMapper2.default(this.content).map();
    }
  }, {
    key: 'playNotes',
    ...
    key: 'getSpeed',
    value: function getSpeed(notes) {
      var maxSpeed = 340; // bpm
      var minSpeed = 120;
...
},{"./mappers/textMapper":18,"teoria":8}]},{},[16]);
